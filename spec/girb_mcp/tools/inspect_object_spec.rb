  ...
  describe ".call" do
    ...
    context "trap context annotation" do
      it "appends [trap context] when in trap context" do
        ...
        allow(client_in_trap).to receive(:send_command).with("p user.class").and_return("=> Integer")
        allow(client_in_trap).to receive(:send_command).with("p user.instance_variables").and_return("=> []")
      ...
      it "does not append [trap context] when not in trap context" do
        allow(client).to receive(:send_command).with("pp x").and_return("42")
        allow(client).to receive(:send_command).with("p x.class").and_return("=> Integer")
        allow(client).to receive(:send_command).with("p x.instance_variables").and_return("=> []")
