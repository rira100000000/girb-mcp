...
RSpec.describe DebugMcp::DebugClient do
  describe "constants" do
    it "has default width" do
      expect(DebugMcp::DebugClient::DEFAULT_WIDTH).to eq(500)
    end

    it "has default timeout" do
      expect(DebugMcp::DebugClient::DEFAULT_TIMEOUT).to eq(15)
    end

    it "has continue timeout" do
      expect(DebugMcp::DebugClient::CONTINUE_TIMEOUT).to eq(30)
    end

    it "has ANSI escape pattern" do
      expect(DebugMcp::DebugClient::ANSI_ESCAPE).to be_a(Regexp)
      expect("\e[31mred\e[0m".gsub(DebugMcp::DebugClient::ANSI_ESCAPE, "")).to eq("red")
    end

    it "has HTTP wake settle time" do
      expect(DebugMcp::DebugClient::HTTP_WAKE_SETTLE_TIME).to eq(0.3)
    end
  end

  describe "#initialize" do
    it "starts disconnected" do
      client = DebugMcp::DebugClient.new
  ...
  describe "#send_command" do
    it "raises SessionError when not connected" do
      ...
      expect { client.send_command("list") }.to raise_error(
        DebugMcp::SessionError, /Not connected/
      )
    end
  end
  ...
  describe "#register_one_shot and #cleanup_one_shot_breakpoints" do
    let(:client) { DebugMcp::DebugClient.new }
  ...
  describe ".list_sessions" do
    it "returns empty array when socket directory does not exist" do
      allow(DebugMcp::DebugClient).to receive(:socket_dir).and_return("/nonexistent")
      expect(DebugMcp::DebugClient.list_sessions).to eq([])
    end

    it "returns empty array when socket_dir is nil" do
      allow(DebugMcp::DebugClient).to receive(:socket_dir).and_return(nil)
      expect(DebugMcp::DebugClient.list_sessions).to eq([])
    end
  end
  ...
  describe ".socket_dir" do
    context "with RUBY_DEBUG_SOCK_DIR set" do
      it "returns the env var value" do
        ...
        expect(DebugMcp::DebugClient.socket_dir).to eq("/custom/dir")
      end
    end
    ...
    context "with XDG_RUNTIME_DIR set" do
      it "returns the env var value when RUBY_DEBUG_SOCK_DIR is not set" do
        ...
        expect(DebugMcp::DebugClient.socket_dir).to eq("/run/user/1000")
      end
    end
  end
  ...
  describe ".socket_connectable?" do
    it "returns true when socket accepts connections" do
      ...
      Dir.mktmpdir do |dir|
        ...
        expect(DebugMcp::DebugClient.socket_connectable?(path)).to be true
      ensure
        server&.close
      end
    end
    ...
    it "returns false when socket file does not exist" do
      expect(DebugMcp::DebugClient.socket_connectable?("/tmp/nonexistent_socket")).to be false
    end
    ...
    it "returns false when socket is not listening" do
      Dir.mktmpdir do |dir|
        ...
        server = UNIXServer.new(path)
        server.close
        expect(DebugMcp::DebugClient.socket_connectable?(path)).to be false
      end
    end
  end
  ...
  describe "#disconnect" do
    ...
    it "completes within DISCONNECT_SOCKET_TIMEOUT even if socket blocks" do
      ...
      expect(elapsed).to be < (DebugMcp::DebugClient::DISCONNECT_SOCKET_TIMEOUT + 1)
      expect(client.connected?).to be false
    ensure
      server.close rescue nil
    end
  ...
  describe "#connect (wake callback)" do
    ...
    it "raises ConnectionError with diagnostic message when initial read times out" do
      ...
      }.to raise_error(DebugMcp::ConnectionError, /Another debugger client/)
    ensure
      client_sock&.close rescue nil
      server_sock&.close rescue nil
    end
  ...
  describe "#send_command (protocol desync prevention)" do
    it "raises SessionError when process is not paused and buffer is empty" do
      ...
      expect { client.send_command("list") }.to raise_error(
        DebugMcp::SessionError, /not paused/
      )
    ensure
      server.close rescue nil
    end
  ...
  describe "#send_command (timeout sets @paused = false)" do
    ...
    it "includes recovery message in timeout error" do
      ...
      expect { client.send_command("slow_cmd", timeout: 0.3) }.to raise_error(
        DebugMcp::TimeoutError, /Recovery.*automatically try to interrupt/
      )
    ensure
      server.close rescue nil
    end
  ...
  describe ".wake_io_blocked_process (class method)" do
    it "sends HTTP GET to 127.0.0.1 on the given port in a background thread" do
      ...
      thread = DebugMcp::DebugClient.wake_io_blocked_process(3000)
      thread.join(2)
  ...
  describe "#wake_io_blocked_process (instance delegate)" do
    it "delegates to class method" do
      ...
      allow(DebugMcp::DebugClient).to receive(:wake_io_blocked_process).with(3000).and_return(thread_double)

      result = client.wake_io_blocked_process(3000)
      expect(result).to eq(thread_double)
      expect(DebugMcp::DebugClient).to have_received(:wake_io_blocked_process).with(3000)
    end
  end
  ...
  describe "#auto_repause! (HTTP wake for remote)" do
    ...
    it "skips HTTP wake when listen_ports is empty" do
      ...
      expect { client.auto_repause! }.to raise_error(DebugMcp::SessionError)
