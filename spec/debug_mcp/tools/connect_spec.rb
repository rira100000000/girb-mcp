...
RSpec.describe DebugMcp::Tools::Connect do
  ...
  describe ".call" do
    before do
      ...
      allow(DebugMcp::DebugClient).to receive(:list_sessions).and_return([])
    end
    ...
    it "passes pre_cleanup_pid from resolved target PID" do
      allow(DebugMcp::DebugClient).to receive(:extract_pid)
    ...
    it "handles connection errors" do
      allow(manager).to receive(:connect).and_raise(
        DebugMcp::ConnectionError, "Connection refused"
      )
    ...
    context "force_reset option" do
      ...
      it "handles no existing session gracefully" do
        ...
        allow(manager).to receive(:client) do |*_args|
          call_count += 1
          if call_count == 1
            raise DebugMcp::SessionError, "No active session"
          else
            client
          end
        end
    ...
    context "error messages suggest force_reset" do
      it "suggests force_reset when connection times out" do
        allow(manager).to receive(:connect).and_raise(
          DebugMcp::ConnectionError, "Connection timed out: the debug process did not respond."
        )
    ...
    context "Docker TCP fallback for listen port detection" do
      ...
      it "falls back to container_web_ports when detect_listen_ports is empty and remote" do
        allow(DebugMcp::TcpSessionDiscovery).to receive(:container_web_ports)
          .with(12345)
          .and_return([3000])
        ...
        expect(DebugMcp::TcpSessionDiscovery).to have_received(:container_web_ports).with(12345)
        expect(text).to include("Listening on:")
        expect(text).to include("http://127.0.0.1:3000")
      end
      ...
      it "does not call container_web_ports for non-remote connections" do
        ...
        expect(DebugMcp::TcpSessionDiscovery).not_to have_received(:container_web_ports)
      end
