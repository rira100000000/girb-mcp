...
RSpec.describe DebugMcp::Tools::GetContext do
  ...
  describe "TRUNCATION_PATTERN" do
    it "matches truncated arrays" do
      expect("x = [1, 2, 3, ...]").to match(DebugMcp::Tools::GetContext::TRUNCATION_PATTERN)
    end

    it "matches truncated hashes" do
      expect('x = {"a"=>1, "b"=>2, ...}').to match(DebugMcp::Tools::GetContext::TRUNCATION_PATTERN)
    end

    it "matches truncated strings" do
      expect('x = "very long string..."').to match(DebugMcp::Tools::GetContext::TRUNCATION_PATTERN)
    end

    it "does not match non-truncated values" do
      expect("x = [1, 2, 3]").not_to match(DebugMcp::Tools::GetContext::TRUNCATION_PATTERN)
    end
  end
