...
RSpec.describe DebugMcp::Tools::Finish do
  ...
  describe ".call" do
    it "runs until method returns" do
      allow(client).to receive(:send_command)
        .with("finish", timeout: DebugMcp::DebugClient::CONTINUE_TIMEOUT)
    ...
    it "handles SessionError for ended sessions" do
      allow(client).to receive(:send_command)
        .with("finish", timeout: DebugMcp::DebugClient::CONTINUE_TIMEOUT)
        .and_raise(DebugMcp::SessionError.new("finished execution", final_output: "last"))
    ...
    it "handles ConnectionError for lost connections" do
      allow(client).to receive(:send_command)
        .with("finish", timeout: DebugMcp::DebugClient::CONTINUE_TIMEOUT)
        .and_raise(DebugMcp::ConnectionError.new("Connection lost", final_output: "output"))
