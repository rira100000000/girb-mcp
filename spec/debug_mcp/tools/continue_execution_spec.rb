...
RSpec.describe DebugMcp::Tools::ContinueExecution do
  ...
  describe ".call" do
    ...
    it "handles SessionError for ended sessions" do
      allow(client).to receive(:send_continue).and_raise(
        DebugMcp::SessionError.new("session ended", final_output: "last output")
      )
    ...
    it "handles SessionError for other errors" do
      allow(client).to receive(:send_continue).and_raise(
        DebugMcp::SessionError.new("Some other error")
      )
    ...
    it "handles ConnectionError for lost connection" do
      allow(client).to receive(:send_continue).and_raise(
        DebugMcp::ConnectionError.new("Connection lost", final_output: "output")
      )
    ...
    it "handles ConnectionError for other errors" do
      allow(client).to receive(:send_continue).and_raise(
        DebugMcp::ConnectionError.new("Connection refused")
      )
    ...
    context "pending HTTP response" do
      ...
      it "appends HTTP response on SessionError" do
        ...
        allow(client).to receive(:send_continue).and_raise(
          DebugMcp::SessionError.new("session ended", final_output: "done")
        )
      ...
      it "appends HTTP response on TimeoutError" do
        ...
        allow(client).to receive(:send_continue).and_raise(DebugMcp::TimeoutError, "timeout")
