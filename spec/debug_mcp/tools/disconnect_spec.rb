...
RSpec.describe DebugMcp::Tools::Disconnect do
  ...
  describe ".call" do
    ...
    it "tries repause then interrupt_and_wait when process is not paused" do
      ...
      allow(client).to receive(:repause).with(timeout: 3).and_raise(DebugMcp::TimeoutError, "timeout")
    ...
    it "auto-escalates with warning and sends resume when both repause and interrupt fail" do
      ...
      allow(client).to receive(:repause).and_raise(DebugMcp::TimeoutError, "timeout")
      allow(client).to receive(:interrupt_and_wait).and_raise(DebugMcp::ConnectionError, "lost")
