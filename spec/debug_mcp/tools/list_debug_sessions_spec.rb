...
RSpec.describe DebugMcp::Tools::ListDebugSessions do
  let(:server_context) { { session_manager: build_mock_manager } }

  before do
    allow(DebugMcp::TcpSessionDiscovery).to receive(:discover).and_return([])
  end
  ...
  describe ".call" do
    ...
    it "lists found unix sessions" do
      ...
      allow(DebugMcp::DebugClient).to receive(:list_sessions).and_return(sessions)
    ...
    it "shows combined unix and TCP sessions" do
      ...
      allow(DebugMcp::DebugClient).to receive(:list_sessions).and_return(unix_sessions)
      allow(DebugMcp::TcpSessionDiscovery).to receive(:discover).and_return(tcp_sessions)
