...
RSpec.describe DebugMcp::Tools::Step do
  let(:client) { build_mock_client }
  let(:manager) { build_mock_manager(client: client) }
  let(:server_context) { { session_manager: manager } }

  describe ".call" do
    it "steps into and returns output" do
      allow(client).to receive(:send_command).with("step").and_return(
        "Stop by #1  BP - Line  file.rb:15 (line)\n=> 15| y = 2"
      )

      response = described_class.call(server_context: server_context)
      text = response_text(response)
      expect(text).to include("file.rb:15")
    end
    ...
    it "handles SessionError for ended sessions" do
      allow(client).to receive(:send_command).with("step").and_raise(
        DebugMcp::SessionError.new("session ended", final_output: "last")
      )
